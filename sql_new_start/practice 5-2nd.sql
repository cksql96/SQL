...
FROM 
    employees, 
    departments;                        --Catesian product

SELECT
    count(*)
FROM
    employees CROSS JOIN departments;   --cross join
...
-- employees(t1)에 있는 컬럼
    last_name,              --decommand
    t1.last_name,           --recommand
-- departments(t2)에 있는 컬럼
    department_name,        --decommand
    t2.department_name,     --recommand
...
-- ------------------------------------------------------
SELECT
    t1.last_name AS "t1.last_name",
    last_name AS "last_name",

    t2.department_name AS "t2.department_name",
    ...
    -- t2.department_id             --X
FROM
    employees t1 JOIN departments t2 USING(department_id);
    -- employees t1 INNER JOIN departments t2 USING(department_id);
...
-- ------------------------------------------------------
SELECT
    t1.last_name AS "t1.last_name",                 --recommand
    last_name AS "last_name",

    t2.department_name AS "t2.department_name",     --recommand
    department_name AS "department_name",

    t1.department_id
    -- department_id    --X
FROM
    employees t1 JOIN departments t2 ON t1.department_id = t2.department_id;
    -- employees t1 INNER JOIN departments t2 ON t1.department_id = t2.department_id;
